...
-- ===================================================================
-- MIGRATION 12 (correção): RPCs de baixa/reversão de combos
-- ===================================================================

-- ===================================================================
-- MIGRATION 12: CorreÃ§Ã£o dos RPCs de baixa/reversÃ£o de combos
-- ===================================================================
-- As queries dos loops FOR referenciam a variÃ¡vel de loop `it` dentro
-- da prÃ³pria consulta (EXISTS ... cm.combo_ref = it.anota_item_ref), o
-- que Ã© invÃ¡lido em PL/pgSQL ("record it is not assigned yet"). Usa-se
...
BEGIN
  SELECT * INTO o FROM public.anota_orders WHERE id = p_order;
  IF o.id IS NULL THEN RAISE EXCEPTION 'Pedido nÃ£o encontrado'; END IF;
  IF o.estoque_aplicado THEN RETURN; END IF;
  IF o.check_status NOT IN (1, 2, 3) THEN RETURN; END IF;
  ...
  LOOP
    ...
    LOOP
      INSERT INTO public.product_movements (product_id, tipo, quantidade, destino, observacoes, user_id, ref_order_id)
      VALUES (r.product_id, 'saida', r.quantidade * it.quantidade, 'Anota AI',
        'Venda Anota AI â€“ Combo '||COALESCE(o.numero, o.external_order_id), p_user, p_order);
    END LOOP;
  END LOOP;
  ...
  LOOP
    INSERT INTO public.product_movements (product_id, tipo, quantidade, destino, observacoes, user_id, ref_order_id)
    VALUES (it.product_id, 'saida', it.quantidade, 'Anota AI',
      'Venda Anota AI â€“ Pedido '||COALESCE(o.numero, o.external_order_id), p_user, p_order);
  END LOOP;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      INSERT INTO public.product_movements (product_id, tipo, quantidade, destino, observacoes, user_id, ref_order_id)
      VALUES (r.product_id, 'entrada', r.quantidade * it.quantidade, 'Anota AI',
        'Cancelamento Anota AI â€“ Combo '||COALESCE(o.numero, o.external_order_id), p_user, p_order);
    END LOOP;
  END LOOP;
  ...
  LOOP
    INSERT INTO public.product_movements (product_id, tipo, quantidade, destino, observacoes, user_id, ref_order_id)
    VALUES (it.product_id, 'entrada', it.quantidade, 'Anota AI',
      'Cancelamento Anota AI â€“ Pedido '||COALESCE(o.numero, o.external_order_id), p_user, p_order);
  END LOOP;
...
-- ===================================================================
-- MIGRATION 13: Grupos de produtos
-- ===================================================================

-- ===================================================================
-- MIGRATION 13: Grupos de produtos (organizaÃ§Ã£o da lista de produtos)
-- ===================================================================
-- Permite criar grupos (ex.: "Fritos", "Assados") e associar produtos a
-- eles, mantendo a lista de produtos organizada por seÃ§Ãµes.
